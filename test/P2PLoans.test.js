...
describe('Clixpesa P2P Loans', function () {
  ...
  before(async () => {
    ...
    addr2 = signers[1]
    addr3 = signers[2]
    console.log("Add1 Bal: ", addr1.address, await Token.balanceOf(addr1.address))
    console.log("Add2 Bal: ", addr2.address, await Token.balanceOf(addr2.address))
    console.log("Add3 Bal: ", addr3.address, await Token.balanceOf(addr3.address))
  ...
  it('Should Create a Loan Request for ADD1', async function () {
    ...
    const requestData = {
      LD: Object.values(requestDetails),
    }
    console.log("Creating a Request: ", requestData.LD[0])
    ...
    expect(results.args.LRD.LD.loanId).to.be.equal(requestData.LD[0])
  ...
  it('ADD2 Should Fund ADD1s Loan Request', async function () {
    ...
    const requestId = add1Requests[0].LD.loanId
    const loanId = "LN" + nanoid()
    const loanAmount = add1Requests[0].LD.principal
    const borrowerBal = await Token.balanceOf(addr1.address)
    await Token.connect(addr2).approve(P2PLoans.address, loanAmount)
    delay(3000)
    console.log("Funding this Request:",requestId)
    const txResponse = await P2PLoans.connect(addr2).fundLoanRequest(requestId, loanId)
    const txReceipt = await txResponse.wait()
    const thisLog = txReceipt.logs.find((el) => el.address === P2PLoans.address)
    const results = P2PLoansIface.parseLog({ data: thisLog.data, topics: thisLog.topics })
  ...
  it('ADD2 Should Create a Request too', async function () {
    ...
    const thisRequest = await P2PLoans.getRequestById(requestData.LD[0])
    expect(thisRequest.LD.initiator).to.be.equal(requestData.LD[2])
  })
  ...
  it('Should Create a Loan Offer for ADD2', async function () {
    ...
    const thisLog = txReceipt.logs.find((el) => el.address === P2PLoans.address)
    const results = P2PLoansIface.parseLog({ data: thisLog.data, topics: thisLog.topics })
    expect(results.args.LOD.LD.loanId).to.be.equal(offerData.LD[0])
